...
def numberOfJournalists(journalists)
  return journalists.length
end

def theShortestHandleOfJournalists(journalists)
sortByShortest = sortArrayByShortest(journalists)
return sortByShortest[0]
end

def howManyHandleWithFiveLetters(journalists)
  justHandleWithFiveLetters = filterArrayByRegExp(journalists,/@\w{5}\b/)
  return justHandleWithFiveLetters.length
end

def howManyHandleStartWithCapitalLetter(journalists)
  justHandleStartWithCapitalLetter = filterArrayByRegExp(journalists,/@[A-Z].*/)
  return justHandleStartWithCapitalLetter.length
end

def filterArrayByRegExp(array,pattern)
  return array.select{|value| value[pattern]}
end

def sortArrayAlphabetically(array)
  return array.sort { |a, b| a.casecmp(b) }
end

def sortArrayByShortest(array)
  return array.sort_by(&:length)
end

def findPositionOfStringInArray(string,array)
  return array.index(string)
end

def distributionByLength(journalists)
  sortedJournalists = sortArrayByShortest(journalists)
  return sortedJournalists.chunk {|handle| handle.length}.each {
